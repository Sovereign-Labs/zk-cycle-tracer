...
struct Args {
    ...
    #[arg(long, required = true)]
    /// Path to the disasembly of the rollup elf
    rollup_dis: String,
...
fn get_cycle_count(insn: &str) -> Result<usize, &'static str> {
    ...
    match insn.to_uppercase().as_str() {
        ...
        | "NEG" | "BNEZ" | "BEQZ" | "BGE" | "BLTU" | "BGEU" | "JALR" | "JAL" | "ECALL"
        | "EBREAK" => Ok(1),
        ...
        "LI" => Ok(2), // LI is a pseudo instruction that expands to LUI and ADDI
...
// Parse the output of `riscv32-unknown-elf-objdump -d -M no-aliases` and inserting a map from `pc`
// to assembley instruction.
fn parse_disasem<'a>(disasem: &'a [u8], instruction_table: &mut HashMap<u64, &'a str>) {
    let mut count = 0;
    let mut line_num = 0;
    'blk: for line in disasem.split(|c| c == &b'\n') {
        line_num += 1;
        if line.len() < 8 || line[0] != b' ' || line[1] != b' ' {
            continue;
        }
        for c in &line[2..8] {
        ...
        let offset: u64 = u64::from_str_radix(str::from_utf8(&line[2..8]).unwrap(), 16).unwrap();
        let instruction = str::from_utf8(&line[29..])
            .unwrap()
            .split_whitespace()
            .next()
            .unwrap();

        if count == 0 || line_num == 333 {
            count += 1;
            println!("{} {}", offset, instruction);
        }
        instruction_table.insert(offset, instruction);
    }
}
...
fn main() -> std::io::Result<()> {
    ...
    // Use `goblin` to build mappings from `pc` to function name and vice versa.
    ...
    // Use the output of `riscv32-unknown-elf-objdump` to build a map from `pc` to instruction
    let disasem = std::fs::read(args.rollup_dis).unwrap();
    let mut instruction_table = HashMap::new();
    parse_disasem(&disasem, &mut instruction_table);

    // Read the execution trace from the file
    ...
    // For each line in the trace, parse the pc and get the instruction.
    // We maintain...
    // - `instruction_counts` to keep track of the number of cycles spent in each function.
    // - A stack trace
    for (c, line) in file_content.lines().enumerate() {
        ...
        parts.next();
        let pc = &parts.next().unwrap_or_default()[3..];
        let pc = u64::from_str_radix(pc, 16)
            .expect(&format!("Failed to parse instruction: {} ({})", line, pc));
        let instruction = *instruction_table
            .get(&pc)
            .expect(&format!("Instruction not found: {} ({})", line, pc));
        ...
        if let Ok(index) = function_ranges.binary_search_by(|&(start, end, _)| {
            ...
            *counts_without_callgraph.entry(fname.clone()).or_insert(0) +=
                get_cycle_count(instruction)
                    .expect(format!("Invalid instruction {}. pc: {}", instruction, pc).as_str());
        } else {
            *counts_without_callgraph
                .entry("anonymous".to_string())
                .or_insert(0) += get_cycle_count(instruction).unwrap();
        }
    ...
    // Print the stack at the final instruction.
    println!("Final function stack: {:?}", function_stack);
